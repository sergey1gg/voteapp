...
export const Login = ({setLoginPage}: {setLoginPage: React.Dispatch<React.SetStateAction<boolean>>}) => {
    const blockRef = useRef<any>(null);
    const [selectedStep, setSelectedStep]= useState<string>("select")
    const [phoneNumber, setPhoneNumber] = useState('');
    useEffect(() => {
      function handleClickOutside(e: any) {
        if (blockRef.current && !blockRef.current.contains(e.target) && !blockRef.current.classList.contains('hidden')) {
          setLoginPage(false)
        }
      }
  ...
  return (
    ...
    <div className="fixed inset-0 z-10 overflow-y-auto">
        ...
        ></div>
        <div className="flex items-center h-auto px-4 py-8">
            <div className="relative w-full max-w-lg p-4 mx-auto bg-white rounded-md shadow-lg">
    <div ref={blockRef} className={`m-auto w-auto  ${selectedStep !=="select"? 'hidden': 'block'}`}>
      <div className="rounded-xl bg-white shadow-xl">
        <div className={`p-6 sm:p-16 `}>
          <div className="space-y-4">
          <svg className="fill-current w-10 inline-block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            ...
            <h2 className="mb-8  text-yellow-600 text-xl font-bold">Sign in to <br /> Vote.</h2>
          </div>
          <div className="mt-16 flex flex-col space-y-4">
            <button className="group h-12 px-6 border-2 border-gray-300 rounded-full transition duration-300 
 hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100"
                onClick={()=> setSelectedStep("whatsapp")}>
              <div className="relative flex items-center space-x-4 justify-center">
                <img src="https://i.pinimg.com/originals/b7/19/87/b7198771222c74b52116be0f7d18b700.png" className="absolute left-0 w-5" alt="whatsapp logo" />
                <span className="block w-max font-semibold tracking-wide text-gray-700 text-sm transition duration-300 group-hover:text-blue-600 sm:text-base">Continue with WhatsApp</span>
              </div>
            </button>
            ...
            <button className="group h-12 px-6 border-2 border-gray-300 rounded-full transition duration-300 
                               hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100">
              <div className="relative flex items-center space-x-4 justify-center">
                <img src="https://upload.wikimedia.org/wikipedia/en/0/04/Facebook_f_logo_%282021%29.svg" className="absolute left-0 w-5" alt="tg logo" />
                <span className="block w-max font-semibold tracking-wide text-gray-700 text-sm transition duration-300 group-hover:text-blue-600 sm:text-base">Continue with Telegram</span>
              </div>
            </button>
          </div>
    ...
    {selectedStep==="whatsapp"? (
        <WhatsApp setSelectedStep={setSelectedStep}/>
      ) : (
        null
      )}
            </div>
